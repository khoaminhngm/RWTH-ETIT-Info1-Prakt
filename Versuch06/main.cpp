...
/**
 * @file main.cpp
 * @brief Praktikumsaufgabe: Verwaltung einer Studentenliste mit dynamischer Datenstruktur (std::vector).
 *
 * Dieses Programm demonstriert den Umgang mit dynamischen Datenstrukturen in C++.
 * Es erlaubt das Verwalten, Bearbeiten, Laden und Speichern einer Studentenliste.
 * Die Datenstruktur basiert auf `std::vector<Student>`.
 *
 * @details Unterstützte Funktionen:
 * - Manuelles oder automatisches Befüllen der Liste
 * - Einfügen am Anfang, Ende oder an beliebiger Position
 * - Löschen nach Position oder Matrikelnummer
 * - Ausgabe vorwärts und rückwärts
 * - Dateiein- und -ausgabe
 * Die `main()`-Funktion implementiert ein Menü zur Verwaltung eines `std::vector<Student>`.
 * Alle Operationen werden interaktiv über die Konsole gesteuert.
 *
